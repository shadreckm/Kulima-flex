...
def build_full_ic_report_text(brief: InvestmentBrief) -> str:
    parts = [build_memo_text(brief), "", "=" * 60, "FULL INVESTMENT COMMITTEE REPORT", "=" * 60, ""]
...
def _build_pdf(brief: InvestmentBrief, full_report: bool = False) -> bytes:
    ...
    sections = [
        ...
        ("6. Investment Recommendation", brief.investment_recommendation or brief.recommendation.value),
    ]
    ...
    if full_report:
        ...
        story.append(Paragraph("Twin Syndicate Investment Committee", styles["KulimaH2"]))
        if brief.syndicate:
            syn = brief.syndicate
            final = syn.final_recommendation or syn.majority_vote
            story.append(
            ...
            if syn.consensus_thesis:
                story.append(Paragraph(_esc(syn.consensus_thesis), styles["KulimaBody"]))
            for v in syn.votes:
                role = v.title or v.persona
                story.append(
                    Paragraph(
        ...
        if brief.future_simulation:
            ...
            if fs.africa_conditions_summary:
                story.append(Paragraph(_esc(fs.africa_conditions_summary), styles["KulimaBody"]))
            for s in fs.scenarios:
            ...
            story.append(Paragraph("Futures simulation not available.", styles["KulimaBody"]))
