...
type ProcessingMode = 'verified' | 'evidence' | 'assisted' | 'demo'
...
const MODES: Array<{ id: ProcessingMode; label: string; description: string }> = [
  ...
  { id: 'demo', label: 'Demo Assessment', description: 'Explore a stored mentor case with no API credits required.' },
]
...
const DEMO_CASES = [
  { useCase: 'Startup', name: 'AgriNova Malawi', result: 'Invest', detail: 'Grade A evidence, institutional corroboration, and one manageable FX risk.', href: '/flex?run=ostx-agrinova-malawi' },
  { useCase: 'NGO', name: 'HealthBridge Lagos', result: 'Review Required', detail: 'Outcome claims are promising, but beneficiary and baseline evidence need verification.', href: '/flex?run=pilot-healthbridge-lagos' },
  { useCase: 'Government Program', name: 'FarmStack Kenya Program Review', result: 'Observe', detail: 'Use the stored program-style case to inspect coverage, operating evidence, and verification gaps.', href: '/flex?run=pilot-farmstack-kenya' },
]
      ...
      <section className="mx-auto max-w-7xl px-5 py-8 md:px-8 md:py-12">
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[minmax(0,1fr)_420px] lg:items-start">
          ...
          <aside className="rounded-[12px] border border-[#0E3627] bg-[#061C14] p-5 text-white shadow-saas-elevated lg:sticky lg:top-6">
            ...
            <div className="mt-5 text-[10px] font-black uppercase tracking-wider text-emerald-300">Processing mode</div><div className="mt-2 grid gap-2">{MODES.map(item => <button key={item.id} type="button" onClick={() => setMode(item.id)} className={`rounded-lg border px-3 py-2 text-left ${mode === item.id ? 'border-emerald-300 bg-emerald-400/15' : 'border-emerald-900 bg-[#0A261C] hover:border-emerald-600'}`}><div className="text-xs font-extrabold text-white">{item.label}</div><div className="mt-0.5 text-[11px] leading-4 text-emerald-100/70">{item.description}</div></button>)}</div>
            <Link href={fileName ? '/api/auth/signin?callbackUrl=%2Fevidence' : '/api/auth/signin?callbackUrl=%2Fflex'} className="mt-5 block rounded-lg bg-[#12B76A] px-4 py-3 text-center text-sm font-black text-[#061C14] hover:bg-[#32D583]">{fileName ? 'Open Evidence Review' : 'Upload Evidence'}</Link><div className="mt-3 text-center text-[11px] text-emerald-200/60">Selected mode: {MODES.find(item => item.id === mode)?.label}</div>
          </aside>
        </div>
        ...
        <section className="mt-14 grid gap-4 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="rounded-[12px] border border-[#DDE6F0] bg-white p-5 shadow-saas"><div className="flex items-end justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-wider text-slate-500">Offline-ready cases</div><h2 className="mt-1 text-xl font-black text-slate-900">Mentor walkthroughs</h2></div><Link href="/mentor-guide" className="text-xs font-extrabold text-[#0B5D3B] hover:underline">Guide</Link></div><div className="mt-5 space-y-3">{DEMO_CASES.map(item => <Link key={item.name} href={item.href} className="block rounded-lg border border-[#DDE6F0] p-3 transition hover:border-[#0B5D3B] hover:bg-[#ECFDF3]"><div className="flex items-center justify-between gap-3"><div><div className="text-[10px] font-black uppercase tracking-wider text-[#0B5D3B]">{item.useCase}</div><div className="mt-1 text-sm font-extrabold text-slate-900">{item.name}</div></div><span className="rounded-full bg-[#F5F8FC] px-2 py-1 text-[10px] font-black uppercase text-slate-700">{item.result}</span></div><div className="mt-2 text-xs leading-5 text-slate-500">{item.detail}</div></Link>)}</div></div>
        </section>
      </section>
    </main>
  )
